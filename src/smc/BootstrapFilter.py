...
class SVBootstrapFilter:
    ...
    def compute_filtering_weights(self, particle, observation, params):
        ...
        log_w = log_gaussian_density_function(X=observation, mean=torch.zeros(size=observation.size()), covariance=torch.exp(torch.tensor(params[2])) * covariance_diag)  # shape (P)
    ...
    def compute_IS_weights(self, resampled_ancestors, particle, next_observation, backward_samples, params):
        ...
        # transition density
        log_w = log_gaussian_density_function(X=particle, mean=params[0]*resampled_ancestors, covariance=torch.exp(torch.tensor(params[1])))
        #log_w2 = manual_log_density_function(X=particle, mean=params[0]*resampled_ancestors, covariance=torch.exp(torch.tensr(params[1])) * torch.eye(1))
        # observation density
        log_w_o = log_gaussian_density_function(X=next_observation, mean=torch.zeros(next_observation.size()), covariance=torch.exp(torch.tensor(params[2])) * torch.exp(particle).unsqueeze(-1))
    ...
    def get_new_particle(self, next_observation, ancestor, weights, params, resampling=True):
        ...
        # Selection : get $h_t$ = \xi_t^l
        particle = params[0] * resampled_ancestor + torch.exp(torch.tensor(params[1]/2)) * torch.normal(mean=ancestor.new_zeros(ancestor.size()), std=ancestor.new_ones(ancestor.size()))
        # compute $w_t$
