...
class RNNBackwardISSmoothing:
    def __init__(self, bootstrap_filter, observations, states, backward_samples, estimation_function, out_folder, index_state=0, save_elements=False):
        # '''
    ...
    def poor_man_smoother_estimation(self):
        with torch.no_grad():
            ...
            h_k_elements = torch.stack([self.estimation_function(k=k, X=self.trajectories[:,:,k,:], index=self.index_state) for k in range(self.seq_len)], dim=-2) # (B,P,S,hidden_size)
    ...
    def plot_estimation_versus_state(self, phi, out_folder):
        ...
        plt.scatter(x, estimation, marker='o', color='cyan', lw=2, label='estimation of X_{}'.format(self.index_state))
        plt.scatter(x, true_state, marker='x', color='green', lw=2, label='ground-truth for X_{}'.format(self.index_state))
    ...
    def plot_multiple_runs(self, phis_backward, phis_pms, out_folder, num_runs):
        ...
        ax.scatter(x, phi_backward_mean, marker='o', color='blue', label='BACKWARD IS: mean estimation of X_{}'.format(self.index_state))
        ax.scatter(xx, phi_pms_mean, marker='o', color='red',
                    label='PMS: mean estimation of X_{}'.format(self.index_state))
        ax.scatter(x, true_state, marker='x', color='green', lw=1, label='ground-truth for X_{}'.format(self.index_state))
        #plt.plot(x, error, 'red', lw=1, linestyle='dashed', label='error for X_{}'.format(self.index_state))
