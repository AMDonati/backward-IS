...
class LSTMCell(nn.LSTMCell):
    ...
    def __init__(self, input_size, hidden_size, p_drop=0, bias=True):
        super(LSTMCell, self).__init__(input_size=input_size, hidden_size=hidden_size, bias=bias)
    ...
    def forward(self, input, hidden=None):
        ...
        gates = F.linear(input=input, weight=self.weight_ih, bias=self.bias_ih) + F.linear(input=hx, weight=self.weight_hh, bias=self.bias_hh) # shape (batch_size, 4 * hidden_size)
...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1, p_drop=0):
        ...
        self.hidden = nn.ModuleList([LSTMCell(input_size=(input_size if layer == 0 else hidden_size),
