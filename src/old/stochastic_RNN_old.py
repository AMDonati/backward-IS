...
class OneLayerRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, bias=True, activation='tanh'):
        ...
        self.sigma_y = 0.5 #TODO: code it as a learnable parameter.
        self.num_particles = 10
        self.batch_size = 16
        self.fc = nn.Linear(in_features=hidden_size, out_features=output_size)
        self.init_smc_params()

    def init_smc_params(self):
        self.resampling_weights = F.softmax(torch.randn(self.batch_size, self.num_particles)) #TODO: to compute it exactly from $h_0$
        self.tau = torch.zeros(1)
    ...
    def backward_IS(self, hy, hx, backward_samples):
        '''get tau_k from ($h_{k}, h_{k-1}, w_{k-1}, tau_{k-1})'''

        # 1. Get backward_indices J
        J = torch.multinomial(self.resampling_weights, num_samples=self.backward_samples)

        # 2. Resample $h_{k_1}$ using J.
        hx_resampled = resample(hx, J) # shape (B,N,H), N = number of backward sample.

        # 3. Compute backward importance sampling weights.
        # l_k = g_k * q_k with $q_k unknown.
        # 4. Compute gradient of log l_k(hx_resampled, hy) for each backward sample.
        # 5. compute $t_k$ with the recursive formula.
    ...
    def forward(self, input, target, hidden=None):
        ...
        for t in range(seq_len):
            ...
            It = torch.multinomial(self.resampling_weights, num_samples=num_particles) #TODO detach I
